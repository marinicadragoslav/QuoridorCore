...
//     : move_counter(0), wx(0), wy(4), bx(8), by(4), wwallsno(10), bwallsno(10), turn('B'), wdists(NULL), bdists(NULL) {
//     for (int i = 0 ; i < 81 ; i++) {
//         walls[i / 9][i % 9] = ' ';
...
Quoridor_state::Quoridor_state(bool startWhite)
    : wx(0), wy(4), bx(8), by(4), wwallsno(10), bwallsno(10), turn(startWhite ? 'W':'B'), wdists(NULL), bdists(NULL), move_counter(0) {
    for (int i = 0 ; i < 81 ; i++) {
        walls[i / 9][i % 9] = ' ';
        if (i < 64) wall_connections[i / 8][i % 8] = false;
    }
...
Quoridor_state::Quoridor_state(const Quoridor_state &other)
    : wx(other.wx), wy(other.wy), bx(other.bx), by(other.by),
      ...
      wdists(NULL), bdists(NULL), move_counter(other.move_counter) {    // TODO: Is it cheaper to copy dists than to potentially recalculate them?
    for (int i = 0 ; i < 81 ; i++) {
...
short int **Quoridor_state::calculate_dists_from(short int x, short int y, bool stop_at_goal, char player) {
     ...
     */
    if (x < 0 || x >= 9 || y < 0 || y >= 9) {
        LOG_INFO(DOM)  << "Error: Invalid coordinates in calculate_dists_from()" << "\n";   // should not happen
        return NULL;
    }
...
int Quoridor_state::get_shortest_path(char player, const Quoridor_move *extra_wall_move, short int posx, short int posy) {
    ...
    if (posx == -1 || posy == -1) {
        if (player == 'W') {
            ...
            LOG_INFO(DOM)  << "Invalid player arg" << "\n";   // should not happen
            return -1;
        }
    }
    ...
    if (extra_wall_move != NULL) {
        ...
        if (extra_wall_move->type != 'h' && extra_wall_move->type != 'v') {
            LOG_INFO(DOM)  << "Error: extra_wall_move is not a wall move!" << "\n";
            return -1;
        }
        if (!legal_wall(extra_wall_move->x, extra_wall_move->y, extra_wall_move->player, extra_wall_move->type == 'h', false)) {   // (!) check_blocking = false to prevent infinite loop
            LOG_INFO(DOM)  << "Error: extra_wall_move is illegal!" << "\n";
            return -1;
        }
...
void Quoridor_state::add_wall(short int x, short int y, bool horizontal) {
    ...
    else { LOG_INFO(DOM)  << "Warning: Illegal wall let through!" << "\n"; }   // should not happen
}
...
bool Quoridor_state::legal_move(const Quoridor_move *move) {
    ...
    {
        LOG_INFO(DOM)  << "Warning: wrong player argument!" << "\n";
        return false;
    }
    if (move->type == 'h' || move->type == 'v') 
    {   // wall move
...
bool Quoridor_state::play_move(const Quoridor_move *move) {
    if (move == NULL || !legal_move(move)) {
        LOG_INFO(DOM)  << "Invalid command: Illegal move: " << ((move != NULL) ? move->sprint() : "NULL") << "\n" << "\n";
        return false;
    }
...
void Quoridor_state::print() const {
    ...
    stringstream ss;
    ss  << "\n" << "  ";
    ss << "     White walls: " <<  wwallsno<< "\n" ;
    ss << "     Black walls: " << bwallsno<< "\n    " ;
    for (int i = 0 ; i < 9 ; i++) 
    {
        ss  << "     " << (char) ('A' + i);
    }
    ss  << "           Player turn: " << (turn == 'W' ? "White" : "Black") << "\n";
    ss << "    +";
    for (int i = 0 ; i < 9 ; i++) {
        ss  << " ━━━ +";
    }
...
//        printf(" %d  ┃", row+1);
        ...
        {
            ss <<"   " << ((bx == row && by == col) ? 'B' : (wx == row && wy == col) ? 'W' : ' ') <<"  "<< (vertical_wall(row, col) ? VWALL : (col < 8 ? "|" : "┃"));
        }
        //printf("  %d", row+1);
         ss << "  " << (row+1);
        // print wall counts
        // if (row < 2) 
        // {
        //     ss << "     " << ((row == 0) ? "White" : "Black") << " walls: " << (row == 0) ? wwallsno : bwallsno;
        ...
        for (int col = 0 ; col < 9 ; col++) 
        {
            if (horizontal_wall(row, col)) 
            {
                ...
                ss << "═════" << string(wall_connections[row][col] ? ((row < 8 && vertical_wall(row, col) && wall_connections[row][col] && vertical_wall(row + 1, col)) ? VWALL : "═") : "+");
            } else if (row < 8) {
                ss << "-----" <<  (vertical_wall(row, col) && wall_connections[row][col] && vertical_wall(row + 1, col) ? VWALL : "+");
            } else {
                ss <<" ━━━ +";
            }
        }
...
Quoridor_move *Quoridor_state::get_best_step_move(char player) {
    ...
    if (argmin == NULL) {
        LOG_INFO(DOM)  << "Warning: Could not find best move in this state:" << "\n";
        this->print();
    }
    list.clear();
    return argmin;
...
double evaluate_position(Quoridor_state &s, bool cheap) {
    ...
    if (max > 0)
        wallsdiff_metric = ((s.wwallsno > s.bwallsno) ? +1 : -1) * ((pow(s.wwallsno - s.bwallsno, 2)) / (pow(max, 2)));
    ...
    double distance_metric = (MAX(path_diff, 10.0)) / 10.0;
...
Quoridor_move *pick_semirandom_move(Quoridor_state &s, uniform_real_distribution<double> &dist, default_random_engine &gen) {
    ...
    LOG_INFO(DOM)  << "Warning: could not find a legal move?" << "\n" << "\n";
}
...
double Quoridor_state::rollout() const {
    ...
    for (int i = 0 ; i < MAXSTEPS ; i++) {
        ...
        if (!s.legal_move(m)) {
            LOG_INFO(DOM)  << "Picked illegal move: " << ((m != NULL) ? m->sprint() : "NULL" ) << " intentionally! Move history:" << "\n";
            #ifdef DDEBUG
            while (!hist.empty()) {
                Quoridor_move *prev = hist.front();
                hist.pop();
                LOG_INFO(DOM)  << prev->sprint() << "\n";
            }
            #endif
        }
        #ifdef DDEBUG
        ...
        if (!noerror) {
            LOG_INFO(DOM)  << "Error: in rollouts" << "\n";
            break;
        }
        #ifndef DDEBUG
        delete m;
